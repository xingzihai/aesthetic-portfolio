...
function initSmartSnap() {
  ...
  const getCurrentSection = () => {
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      // 判断 section 是否在视口中
      if (sectionTop <= scrollTop + viewportHeight * 0.5 && 
          sectionBottom > scrollTop + viewportHeight * 0.5) {
        return i;
      }
    }
    return 0;
  ...
  // 判断当前 section 是否滚动到底部
  const isAtSectionBottom = (section) => {
    const rect = section.getBoundingClientRect();
    const viewportHeight = window.innerHeight;
    
    // section 底部是否接近视口底部（允许 10px 容差）
    return rect.bottom <= viewportHeight + 10;
  };
  
  // 判断当前 section 是否滚动到顶部
  const isAtSectionTop = (section) => {
    const rect = section.getBoundingClientRect();
    // section 顶部是否接近视口顶部（允许 10px 容差）
    return rect.top >= -10 && rect.top <= 10;
  };
  
  // 判断 section 内容是否超出一屏
  const isContentOverflow = (section) => {
  ...
  const toggleSnap = (enable) => {
    ...
    if (enable) {
      html.classList.remove('snap-disabled');
    } else {
      html.classList.add('snap-disabled');
    }
  };
  ...
  const handleScroll = () => {
    ...
    const contentOverflow = isContentOverflow(currentSection);
  ...
  // 滚动结束检测
  const onScrollEnd = () => {
    // 滚动结束后，重新启用 snap（确保对齐）
    const currentSection = sections[currentSectionIndex];
    if (currentSection && !isContentOverflow(currentSection)) {
      toggleSnap(true);
    }
  };
